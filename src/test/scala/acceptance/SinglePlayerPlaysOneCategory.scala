...
class SinglePlayerPlaysOneCategory extends UnitSpec {
	...
	"single player plays one category" in {
		...
		verify(console) printLine("Dices: D1:2 D2:4 D3:1 D4:6 D5:1")
		verify(console) printLine("Dices to re-run: D1 D2 D4")
		verify(console) printLine("Dices: D1:1 D2:5 D3:1 D4:2 D5:1")
		verify(console) printLine("Dices to re-run: D2 D4")
		verify(console) printLine("Dices: D1:1 D2:1 D3:1 D4:5 D5:1")
